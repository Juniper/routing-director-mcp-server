...
| `auth.token` | No | JRD API Token for authentication (if using instead of user/pwd) |
| `openapi_spec` | No | Path to Routing Director OpenAPI spec 3.0 (JSON)                |
| `components` | No | List of components to include in the MCP server (e.g., `["ems", "juniper-resiliency-interface"]`) |

Please note: 
* For authentication, you can either use `username` and `password` for basic auth or provide an `auth.token` if your Routing Director instance is configured to accept API tokens.
* `openapi_spec` and `components` are optional fields. `openapi_spec` is used to provide Routing Director's Openapi spec (json). If the user wishes to filter the openapi spec only for certain components, then the `components` field can be used to provide a list of components to filter the spec.
...
If components field is not provided, then all the above components will be included in the MCP server by default.
...
```bash
python token_cli.py generate --client-id dynamic-test-2
```
